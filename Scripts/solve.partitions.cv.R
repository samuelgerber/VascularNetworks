...
for(k in 1:n.repeats){
  ...
  data.train = data.frame( x = mds2$points[index.train,1], gender = as.factor(labels[index.train,2]) )
  data.test  = data.frame( x = mds2$points[index.test,1] , gender = as.factor(labels[index.test,2]) )
